import { r as reactExports, j as jsxRuntimeExports, b as api } from "./index-C8l_MEH7.js";
...
const GerenciarResponsaveisModal = ({ isOpen, onRequestClose, eventId, organizadores }) => {
  ...
  const fetchUsers = async (query) => {
    ...
    try {
      ...
      const response = await fetch(`${"http://18.216.192.230/api"}/users/search?identifier=${query}`, {
        headers: {
  ...
  const handleInativarOrganizador = async (organizadorId) => {
    try {
      const token = api.get("authToken");
      await fetch(`${"http://18.216.192.230/api"}/events/${eventId}/organizadores/${organizadorId}`, {
        method: "DELETE",
        headers: {
  ...
  const handleAdicionarOrganizador = async (user) => {
    ...
    try {
      const token = api.get("authToken");
      await fetch(`${"http://18.216.192.230/api"}/events/${eventId}/organizadores`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
