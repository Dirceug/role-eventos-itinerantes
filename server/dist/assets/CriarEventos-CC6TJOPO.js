import { r as reactExports, U as UserContext, u as useNavigate, j as jsxRuntimeExports, b as api } from "./index-HoM017Nb.js";
import { l as lodashExports } from "./lodash-CyKDKuZ6.js";
import { a as axios } from "./index-BSBq6A-N.js";
import { J as Joi } from "./joi-browser.min-DoVLzfa9.js";
import BackButton from "./BackButton-B_yyfa93.js";
...
const EventForm = () => {
  ...
  const toggleEditResponsaveisBarraca = reactExports.useCallback(() => {
    setIsEditingResponsaveisBarraca((prev) => !prev);
  }, []);
  const toggleEditFuncionariosBarraca = reactExports.useCallback(() => {
    setIsEditingFuncionariosBarraca((prev) => !prev);
  }, []);
  const addResponsavelBarraca = reactExports.useCallback((responsavel) => {
  ...
  const addFuncionarioBarraca = reactExports.useCallback((funcionario) => {
  ...
  const fetchUsers = async (query) => {
    ...
    try {
      ...
      const response = await fetch(`${"https://18.216.192.230/api"}/users/search?identifier=${sanitizedQuery}`, {
        headers: {
  ...
  const handleEnderecoChange = reactExports.useCallback((field, value) => {
  ...
  const addBarraca = reactExports.useCallback(() => {
    setBarracas((prev) => [
      ...prev,
      {
        nome: "",
  ...
  const updateBarraca = reactExports.useCallback((barracaIndex, field, value, pratoIndex = null) => {
    setBarracas((prev) => {
      const updated = [...prev];
  ...
  const addPrato = reactExports.useCallback((barracaIndex) => {
    setBarracas((prev) => {
      const updated = [...prev];
  ...
  const removePrato = reactExports.useCallback((barracaIndex, pratoIndex) => {
    setBarracas((prev) => {
      const updated = [...prev];
